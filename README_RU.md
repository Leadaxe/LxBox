...
- **Триггеры**: запуск app · через 2 мин после VPN connected · раз в час · сразу по VPN disconnected · manual ⟳ (force)
...
Multi-hop цепочки: трафик идёт через промежуточный сервер перед финальным прокси. Отлично для обхода гео-ограничений зарубежного провайдера: поставьте домашний WireGuard как detour → заграничный мобильный интернет превращается в тоннель к дому.
...
<details>
<summary><strong>Обход DPI</strong> — обход цензуры</summary>

Три ортогональных трюка — комбинируются на одном outbound.

- **TLS Fragment** — разбивает ClientHello по TCP-сегментам
- **TLS Record Fragment** — разбивает handshake на несколько TLS-записей
- **Mixed-case SNI** (v1.3.0+) — рандомизирует регистр `server_name` (`WwW.gOoGle.CoM`). Ломает наивный exact-match DPI у региональных провайдеров. По RFC 6066 SNI case-insensitive; трюк не меняет поведение сервера. Не работает против GFW-класса.
- Все трюки применяются только к первому хопу (внутренние — в туннеле, локальный DPI их не видит).
...
<details>
<summary><strong>Haptic feedback</strong> — вибро на VPN-события</summary>

Короткая вибрация на transitions VPN, ошибках и тапах. Респектит системную настройку Android Touch feedback.

- Tap Start/Stop → лёгкий tick
- VPN connected → средний impact; user disconnect → лёгкий
...
Организованы по секциям: General, Clash API, Network, Include Auto, DNS, TUN, DPI Bypass. URLTest параметры для авто-подбора прокси. Все изменения автосохраняются.
</details>
