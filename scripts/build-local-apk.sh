...
# Корень репо запоминаем ДО `cd app` ниже: trap срабатывает уже из app/, и
# относительный путь app/pubspec.yaml там не существует — восстановление
# молча не отрабатывало, в дереве оставалась подменённая версия.
REPO_ROOT="$(pwd)"
trap 'git -C "$REPO_ROOT" checkout -- app/pubspec.yaml 2>/dev/null || true' EXIT
