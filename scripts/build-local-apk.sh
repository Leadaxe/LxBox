...
# git-хуков нет). Этот скрипт переписывает `version:` строку перед build'ом;
# в дереве после сборки останется вычисленная строка — это ожидаемо (git-diff
# на pubspec безвреден, значение не читается runtime — PackageInfo берёт из APK).
