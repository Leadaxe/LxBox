...
const _knownDivergence = <String, bool>{
  ...
  // ОБА отвергают, и оба — вопреки корпусу: рантайм обязан дать вырожденно
  // ложный `or` (expected.json: hit=false, warnings пуст), а валидаторы
  // требуют непустой список. Расхождения между приложениями тут нет —
  // расхождение валидаторов с рантаймом. Кандидат на `"load": "accept"`
  // и правку обеих сторон.
  ...
  // Dart принимает, Go отвергает: `#in`/`#notIn` на enum-var. Go считает
  // операторы неприменимыми к enum, Dart — применимыми.
