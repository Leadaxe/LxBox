...
void main() {
  ...
  // §128 — «Force direct-out»: overrideDetour=direct-out + replace=true.
  // Та же REPLACE-машинерия, что и target='jump-out' выше, но целевой тег —
  // зарезервированный direct-outbound. Цепочка выкидывается (skipDetour).
  group('§128 — force direct-out', () {
    test('main.detour = direct-out, config валиден (direct-out существует)',
        () async {
      final spec =
          parseUri('vless://u1@h1.com:443?type=ws&security=tls#FD')!;
      final list = UserServer(
        id: 'fd1',
        name: 'Test',
        enabled: true,
        tagPrefix: '',
        // Ровно то, что пишет node_settings dropdown «Force direct-out».
        detourPolicy: const DetourPolicy(
          overrideDetour: kDirectOutTag,
          replaceDetourChain: true,
        ),
        origin: UserSource.paste,
        createdAt: DateTime.now(),
      ...
      expect(result.validation.isOk, true,
          reason: result.validation.issues.join('\n'));
      final outs = (result.config['outbounds'] as List).cast<Map>();
      final main = outs.firstWhere((o) => o['tag'] == 'FD');
      expect(main['detour'], kDirectOutTag);
      // direct-out реально существует в конфиге (no dangling ref).
      expect(outs.map((o) => o['tag']).toSet().contains(kDirectOutTag), true);
    });

    test('force-direct поверх узла с native detour-цепочкой → цепочка выкинута',
        () async {
      // Узел с зашитым нативным detour (multi-hop через jump-out внутри
      // raw-конфига). Force-direct (REPLACE) должен ОТБРОСИТЬ её и оставить
      // только direct-out.
      final spec = parseUri(
          'vless://u1@h1.com:443?type=ws&security=tls#FDChain')!;
      final list = UserServer(
        id: 'fd2',
        name: 'Test',
        enabled: true,
      ...
      expect(result.validation.isOk, true,
          reason: result.validation.issues.join('\n'));
      final outs = (result.config['outbounds'] as List).cast<Map>();
      final main = outs.firstWhere((o) => o['tag'] == 'FDChain');
      // direct-out, а НЕ цепочка.
      expect(main['detour'], kDirectOutTag);
    });
  });
}
