...
///
/// §474 (контракт 1.1.7) — сведение МУСОРА к `auto` отсюда СНЯТО.
///
/// Запрос шага 4 лаунчер закрыл в свою пользу: подмена перестала молчать. У
/// `security` теперь свой код `vmess_security_unknown` (warning) вместо общего
/// `type_invalid`, и ставит его реестр (`body.fields.security`: enum +
/// `on_invalid: coerce auto`). Текст `type_invalid` — «поле снято: неверный
/// тип» — про этот случай врал вдвойне: поле не снимается, а подменяется, и
/// узел уезжает не на том шифре, который просила подписка. Молчать о таком
/// нельзя, а ожидание корпуса (`uri/vmess/vmess_security_ctr`) дополнено
/// кодом.
///
/// Здесь остались ровно два случая, суждением о значении НЕ являющиеся:
///
/// 1. `chacha20-ietf-poly1305` → `chacha20-poly1305` — перевод диалекта Xray,
///    имя того же шифра (`uri.query.scy.impl`). Кода не даёт, как и прочие
///    переводы написания;
/// 2. пусто / `'null'` / `'undefined'` строкой (их шлют реальные панели,
///    корпус `json_scy_null_to_auto`, `json_security_key_null`) → `auto`.
///    «Не задано» — не выбор автора ссылки, предупреждать не о чем.
///
/// Ключ `security` обязан ПОЯВИТЬСЯ в теле и в этом случае: у ядра поле без
/// `omitempty`, у схемы оно `required`, а `default` реестра тело не наполняет
/// (материализуется только `default_when`, CANON §2.4). Опущенный ключ уронил
/// бы узел кодом `field_missing` — на этом лаунчер споткнулся в первом заходе
/// и предупредил отдельно.
///
/// Всё прочее уезжает в тело как приехало (в нижнем регистре), и вердикт
/// выносит санитайзер — одинаково на всех входах.
