...
///
/// `null` — ссылки нет вовсе (текст не разбирается в URI).
///
/// §481 (контракт 1.1.11) — ПУСТОЙ host маппер больше не судит сам. Ядро на
/// пустом адресе валит ВЕСЬ конфиг («invalid server address»), и узел обязан
/// исчезнуть — но исчезает он теперь по правилу реестра (`server` обязателен),
/// а значит С КОДОМ `field_missing`, который нормирует `dropped[].code`
/// корпуса. Прежде маппер ронял его молча, и конверт отбраковки приезжал без
/// кода: отличить «пустой адрес» от любой другой причины было нечем.
