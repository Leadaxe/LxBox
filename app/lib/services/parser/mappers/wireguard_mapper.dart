...
UriMapping? mapWireguardUri(String uri) {
  ...
  final privateKeyRaw = (p.userInfo.isEmpty
  ...
  final peer = <String, dynamic>{
    ...
    'allowed_ips': _cidrList(q['allowedips'] ??
        q['allowed_ips'] ??
        _kDefaultAllowedIps),
  ...
  // §463/§473 — РОД УЗЛА ПО ЗАПРОСУ ССЫЛКИ, а не по уцелевшим полям.
  //
  // Условие `when.any_set` реестра судит НАЛИЧИЕ КЛЮЧА в теле, и для почти
  // всех AWG-узлов этого довольно: хоть одно поле да уцелело. Но ссылка может
  ...
  // неотличимо, а корпус ждёт `mtu: 1280`: ссылка просила AWG, и потолок —
