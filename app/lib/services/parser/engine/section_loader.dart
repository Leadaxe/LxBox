...
final class MapperSections {
  ...
  ///
  /// **`when` у записи секции изъятия НЕ отменяет — РАСХОЖДЕНИЕ с
  /// MAPPER_ENGINE §7.1, оставленное осознанно (§532 дефект 3).** Норма
  /// говорит: условная запись переопределением не считается, потому что
  /// исполняется лишь в части случаев. Go так и делает
  /// (`linkmap/plan.go:529`). У нас это правило меняет ТЕЛО живого узла: у
  /// одной из QUIC-схем запись `sni` условна (гейт по `query.disable_sni`), и
  /// при её неизъятии запись блока с тем же набором `source` доживает до стадии
  /// дефолтов, где её `default_from: host` возвращает `tls.server_name`,
  /// снятый записью `disable_sni`. Кейс `b480:disable_sni_with_sni` меняет
  /// identity — у пользователей слетают выбор узла, отключения и цепочки.
  /// Правило берётся вместе с разделением блока на стороне реестра (задача
  /// 533 после синка 1.1.53), а не раньше него.
