...
final class _Run {
  ...
  bool _applyUserinfo() {
    ...
    if (sep == null || !raw.contains(sep)) {
      ...
      //
      // Умолчание ниже, однако, СНЯТО БЫТЬ НЕ МОЖЕТ, пока оно исполняется:
      // живой реестр (`assets/contract`, зеркало лаунчера) объявляет
      // `single_into` только там, где поле НЕ первое, а у четырёх секций с
      // двумя именами полагается ровно на это умолчание. Снять его здесь
      // значило бы обнулить их первое поле — identity живых узлов, — правкой
      // в чужом зеркале, которую наша сторона не делает. Умолчание уходит
      // вместе с синком реестра, где `single_into` проставлен явно всем.
      //
