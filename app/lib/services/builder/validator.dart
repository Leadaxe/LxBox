...
ValidationResult validateConfig(Map<String, dynamic> config) {
  ...
  // набором виновников.
  //
  // §393 A4 — это уже ПОСЛЕДНИЙ рубеж, а не первый: политика приведена к
  // лаунчеру, и штатные кольца рвёт граф-санитайзер
  // (`post_steps/sanitize_outbound_graph.dart`, правила 4-5) ДО валидатора —
  // деградацией с warning. Сюда доезжает только то, чего санитайзер развязать
  // не смог (ни одно removable-ребро не разваливает кольцо), и тогда прежний
  // §254-контракт в силе: конфиг не собирается, юзер устраняет причину сам.
...
///
/// §393 A4 — вызывается ПОСЛЕ граф-санитайзера, который штатные кольца уже
/// разорвал: на нормальном пути этот детектор возвращает пустой список.
/// Алгоритм минимальности при этом никуда не делся — санитайзер переиспользует
/// его же (`_cyclicGraphNodes` в `sanitize_outbound_graph.dart`), чтобы резать
/// то ребро, что развязывает кольцо, а не первое попавшееся.
///
