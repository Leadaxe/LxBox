...
// (директива оператора 24.08; так же у лаунчера). §439: хранение это
// повторяет — записи `kind: chain` лежат в `sources[]` ХВОСТОМ, после записей
// подписок, серверов и папок, в своём порядке (BACKUP §4: порядок записей
// нормативен). Отдельного ключа и поля позиции нет.
//
...
// часть цепочек, не трогая остальное.
//
// Инвариант «позиция вправе сослаться только на цепочку ВЫШЕ» считается по
// взаимному порядку цепочек: перетащить подписку МЕЖДУ двумя цепочками
// законно, ни одна ссылка от этого не ломается. Экран рисует цепочки после
// записей контроллера — там же, где они лежат.
...
/// Записать цепочки хвостом `sources[]` в порядке [chains]; записи прочих
/// источников остаются на местах как лежат.
Future<void> _setChains(List<SourceChain> chains, {bool flush = true}) async {
  ...
  data[kSourcesKey] = [
    ...
    for (final c in chains) chainToRecord(c),
  ];
