...
class _BackupScreenState extends State<BackupScreen> with SnackHelper {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text(context.l.backupTitle)),
      body: ListView(
  ...
  Future<void> _onExport() async {
    ...
    if (include.isEmpty) {
      showSnack(context.l.backupNothingToExport);
      return;
    }
    setState(() => _busy = true);
    try {
  ...
  Future<void> _onImport() async {
    ...
    try {
      ...
      if (raw == null) {
        if (!mounted) return;
        showSnack(context.l.backupCouldNotReadFile);
        return;
      }
      ...
      try {
        ...
        if (!mounted) return;
        _showError(context.l.backupInvalidTitle, e.message);
        return;
      }
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            action: parts.isEmpty
                ? null
                : SnackBarAction(
                    label: context.l.backupRestartNow,
                    onPressed: () =>
