...
- ⚠️ Установка не выполнена: телефон `192.168.1.71:5555` = **offline** (adb
  отвалился при работающем VPN). Нужен USB-реконнект / повторная авторизация.

Дальше: P6 чистка → P7 ARCHITECTURE.md → P8 (install бинаря когда телефон
вернётся) → §090.
