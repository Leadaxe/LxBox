...
## Device-чеклист (CPH2411) — ✅ пройден 01.08.2026

Локальная сборка проверена на устройстве, регрессий не найдено. Проверялось:

- [x] холодное открытие экрана Config: без фриза (парс в изоляте), скролл плавный;
- [x] ввод/удаление символов в середине документа: без лагов, без роста памяти;
- [x] paste большого куска, undo/redo;
- [x] long-press: контекстное меню copy/paste (свой `toolbarController`);
- [x] кириллица в комментариях JSON5 — ввод и загрузка из файла (utf8-фикс);
- [x] swipe-клавиатура (Gboard glide) не сходит с ума на IME;
- [x] Save 200 КБ: без ANR/джанка, конфиг применяется;
- [x] Source-таб подписки на 1000 нод: raw и decoded (§302) — открытие без фриза;
- [x] просмотр большого краш-репорта (сотни КБ) — скролл, Copy;
- [x] wizard: вставка 1000 `vless://`-ссылок в URI-поле — поле живое, Import работает;
- [x] память до/после открытия редактора — нет кратного роста.

Не проверено полевым тестом (не требует ретеста, но стоит помнить): конфиг
свыше порога 1 МБ — read-only-баннер отрабатывает по unit-гейту
`configTooLargeToEdit`, вживую такой файл не собирался.
