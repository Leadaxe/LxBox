...
## §7 — верификация контракта ядра (открыт 1 пункт из 4)
...
1. **Открытый (единственный незакрытый)**: поймать live-`failed`-событие
   от `cloudflare_udp` и убедиться, что `outbound` **пуст** (подтвердить, что у
   дефолтного сервера нет `detour` в рабочем конфиге). Если пришло
   `["direct-out"]` — формула всё равно корректна (ветка «содержит direct»), но
   факт зафиксировать. Снимается глазами на устройстве через debug-хендлер
   DNS-стрима (`domain / source / rcode / outbound / error`).
