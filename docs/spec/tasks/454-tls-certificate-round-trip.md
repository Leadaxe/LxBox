...
- `ech` — не читается и не эмитится: ядро без `with_ech` (D-006), LxBox
  ...
  **Пересмотрено §459: `tls.ech` из JSON проходит** — посылка D-006 оказалась
  ложной (`common/tls/ech_tag_stub.go` объявляет тег устаревшим, ECH
  компилируется всегда), объект добавлен в allowlist как сквозной
  ключ-объект; снимается только URI-параметр `ech=` Xray-формы.
