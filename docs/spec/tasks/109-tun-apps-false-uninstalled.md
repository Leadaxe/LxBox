...
| Поле | Значение |
|------|----------|
| Статус | Done — в develop, установлено на тест-телефон (vc 2521); визуальная проверка таба за юзером |
| Дата старта | 2026-06-10 |
| Дата завершения | 2026-06-10 |
| Коммиты | `6972fe5` fix(§109): Tunnel apps — ложный «uninstalled, auto-skipped» |
...
- Установлено на тест-телефон 2026-06-10: `adb install -r` поверх,
  `versionCode 2521` (бамп 2520→2521 `--build-number` одобрен юзером —
  на телефоне стоял локально бампнутый 2520, честный git-count ~534 не
  встаёт как downgrade), приложение запустилось. ✅
- Визуальный smoke (таб Tunnel apps: установленные — без меток, иконки
  подгружаются; реально удалённый пакет — с меткой) — за юзером.
