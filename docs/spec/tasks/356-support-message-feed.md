...
**Статус:** реализовано и DEVICE-VERIFIED 03.08.2026 (юниты 23/23, полный сьют 2754; эмулятор sdk_gphone64_arm64: очередь №1→№2, «Got it»/«Later», ru/en-локаль в момент показа, baseline/read в support_state.json, native-uptime доливка session_credited; найден и починен фейл единственной попытки fetch на первом тике connected → ретрай с бэкоффом 30с)
...
**Fetch — до первого успеха, с бэкоффом 30с** (не «однократ за процесс», как
v1): единственная попытка сгорала ровно в самый ненадёжный момент — первый
тик connected, когда туннель поднят, но нода ещё не пропускает трафик
(device-verified на эмуляторе: fetch-фейл + v1-кэш → лента молчала до
перезапуска процесса). Успешный feed держится в памяти процесса, дальше не
фетчим.
