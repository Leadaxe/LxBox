...
| Поле | Значение |
|---|---|
| Статус | complete — П1/П2/П3 + инспекция F1–F7 (код+тесты+доки) |
...
- [x] П1: `AUTOMATION.md` содержит чеклист «событие не приходит» (первым — гейт
      категории State), явный шаг объявления экстр в Tasker + список ключей
      события + пометку про пустой `old_tag` на первом switch.
- [x] П2: `AUTOMATION.md` описывает плагин-условие `active-node` (условие
      **Active node =**) как pull-ответ на «узнать текущую ноду».
...
П1/П2 дописаны в `AUTOMATION.md` (чеклист «событие не приходит» + объявление
экстр в Tasker + пометка про пустой `old_tag`; условие **Active node =** как
pull-ответ «узнать текущую ноду»).

Осталось: `urltest-group`-дубль (нарушение общей базы) — отдельной таской.
F8/F9 — задокументированы как известные. Device-verify (no-op guard + F1/F3 на
реальном Tasker) — pending.
