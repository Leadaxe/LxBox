...
| Core | sing-box-lx `v1.14.0-lx.34` (пин на момент релиза; device-verify фичи прошёл на `v1.14.0-lx.33` — AWG3-слой между ними не менялся, `lx.34` — переезд на стабильную базу апстрима 1.14.0). Минимум для AWG3 — `lx.32` (первое ядро с полями), `lx.33` чинит приём data-пакетов при `random_trailers`; ядро ≤ lx.31 отвергает конфиг с любым AWG3-ключом целиком. Справочник полей: `sing-box-lx/docs-lx/lx-protocols-transports.ru.md` §2.1, §2.7, §2.9, §2.10 |
