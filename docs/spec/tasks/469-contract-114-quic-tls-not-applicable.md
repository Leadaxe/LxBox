...
> **СНЯТО у hysteria2 — §472 шаг 5.** Предсказание сбылось ровно так,
...
> - **на пути ссылки** — вместе с самим парсером: `hysteria2_parser.dart` стал
>   строкой вызова конвейера;
> - **на пути тела hysteria2** (`json_parsers.dart`) — как лишний, а не
...
>
> Функция `forbiddenTlsBlockWarnings` жива для **tuic** (переезжает вторым
> коммитом этого же шага), **masque** и **hysteria v1** (шаг 7). Подробности — спека 472, раздел 11.
