...
- В списке узлов (`node_row.dart`) у выключенного узла в сабтайтле, на месте
  `up`/`sleep`/`down`: `● off` — точка и текст `Colors.orange` (как пинг
  200–500 мс), `off` курсивом. Справа вместо пинга прочерк `—`
  (onSurfaceVariant). При нехватке ширины обрезается текст, не точка.
