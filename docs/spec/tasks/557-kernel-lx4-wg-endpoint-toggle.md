...
| Поле | Значение |
|------|----------|
| Статус | **ТЗ**, решения владельца приняты 26.09.2026: А1, Б2 |
| Дата | 2026-09-26 |
...
- Узел, который сейчас выбран в каком-либо selector: пункт «Turn off» и
  переключатель недоступны (disabled) с подсказкой «Selected in <group>» (решение Б).
  Включить уже выключенный узел можно всегда.
...
- **Б2.** Выбранный в selector узел выключить нельзя. Если узел уже выключен
  и потом его выбирают в selector — выбор не блокировать, узел остаётся
  выключенным (пользователь видит «Off» рядом).
