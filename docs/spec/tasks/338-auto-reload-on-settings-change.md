...
| | |
|---|---|
| Статус | ✅ Реализовано, DEVICE-VERIFIED (эмулятор 02.08.2026: сценарий «DNS-правило вкл/выкл → возврат» — плашка не залипает; телефон: авто-reload на реальной правке подтверждён логом `changed=true → stale → reload ok`) |
| Дата | 2026-08-02 |
...
### Корень репорта: гонка dispose-флаша LazyPersistMixin (5)

Точная репродукция юзера — «тумблер DNS-правила → возврат → синяя плашка» —
вскрыла пятый, главный дефект, и он **старше §338**:

```
тумблер правила        → markDirty: флаг=true, данные в staged-кэш
возврат (didPop)       → пересборка стартует, staged-данные видит ✓
пересборка закончила   → флаг=false
dispose экрана (~300мс,
после exit-анимации)   → _flush → ПОВТОРНЫЙ stageChanges() (safety-net)
                         → typed-saver внутри зовёт markConfigDirty()
                         → флаг=true — плашка при актуальном конфиге
```

Гонка с exit-анимацией: пересборка медленнее ~300мс — её сброс затирал
ре-подъём (везло); быстрее — плашка залипала. Отсюда «иногда». Побочно
dispose-флаш писал настройки на диск после конфига → mtime настроек новее →
следующий старт восстанавливал dirty mtime-сравнением (§076) — плашка
«возвращалась» и после рестарта приложения.

Фикс: `_flush` больше не перезапускает `stageChanges()`, а ждёт
`_stagingInFlight` — staging-future последней мутации (данные в кэше уже с
момента тыка). Ре-подъёма нет по построению, restore-хаков не потребовалось.
Затрагивает все экраны на миксине: dns_settings, routing, tun_apps, vpn_mode.

До §338 баг жил незамеченным: юзер и так жал плашку руками, лишняя не
удивляла. Галка «плашек не должно быть вовсе» его высветила.

Диагностика: трассер `configDirty` — переход false→true логируется с
caller-фреймами (setter в `SettingsStorage`). Оставлен в коде насовсем.
