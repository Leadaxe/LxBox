...
- **`--dart-define` первичен там, где применим** — в момент сборки мы точно
  знаем адресата артефакта. Прецеденты живые: `LXBOX_SUPPORT_URL` (§356) и
  `DONATE_URL` читаются через `String.fromEnvironment`, `LXBOX_ABI_FILTER`
  прокидывается через env в `build.gradle.kts`.

  ⚠ По факту флаг ставится **только на AAB** (Play). У APK его нет — он сломал
  бы сверку байтов F-Droid'а с GitHub-релизом (reproducible builds), см.
  «У APK define НЕ ставится» ниже. Для APK работает только рантайм-ветка.
...
- **Рантайм — фолбэк** (а для APK — единственный путь). `installingPackageName`
  врёт в обе стороны: sideload
...
| `ci.yml`, шаг «Build APKs» | **ничего** — см. ниже |
| `fdroiddata`, build-блоки | **ничего** — см. ниже |
| `scripts/build-local-apk.sh` | не трогаем — фолбэк даст `github`, что верно |

### ⚠ У APK define НЕ ставится — reproducible builds

Первая редакция спеки требовала `=github` в APK-шагах и `=fdroid` в рецепте.
**Отменено при реализации** — это ломало воспроизводимость.

Все три build-блока рецепта несут `binary:` — ссылку на APK из GitHub
Releases. F-Droid собирает из тех же исходников и **сверяет байты**: при
совпадении приложение раздаётся под нашей подписью вместо F-Droid'овской
(и остаётся обновляемым поверх GitHub-сборки). `--dart-define` попадает в
скомпилированный Dart-код, поэтому «GitHub-APK с `=github`» и «F-Droid-APK с
`=fdroid`» дали бы разные байты и сверка бы упала.

Решение: **у APK канала в сборке нет**, он определяется рантаймом по
`installingPackageName`:

| Как поставлено | installer | Канал |
|---|---|---|
| APK со страницы релиза (браузер/файловый менеджер) | `null` / пакет менеджера | `github` ✓ |
| Клиент F-Droid | `org.fdroid.fdroid` | `fdroid` ✓ |

Обе ветки верны без всякого флага. Define остаётся только у **AAB**: Play
раздаёт отдельный артефакт, с ним `binary:` не сверяется.

Побочный эффект, признанный правильным: APK, скачанный с GitHub и поставленный
**через клиент F-Droid**, определится как `fdroid` — и это верно, обновлять его
будет именно F-Droid.
...
⚠ Define использовался **как инструмент верификации**, чтобы прогнать три
канала на одном устройстве. В проде он остаётся только у AAB (Play): у APK
канал резолвится рантаймом — см. «У APK define НЕ ставится». Проверка от этого
не обесценивается: define и рантайм сходятся в одной точке
(`InstallSource.current`), а ветка рантайма покрыта unit-тестами и
подтверждена здесь же строкой «пакет ставился через `adb`».
...
| `docs/FDROID.md` | «Чем сборка отличается» — предупреждение, что define в рецепт добавлять НЕЛЬЗЯ (ломает сверку `binary:`), канал резолвится рантаймом |
