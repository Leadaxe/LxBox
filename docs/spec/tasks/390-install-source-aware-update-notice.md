...
| Кейс | Как проверить |
|---|---|
| `github` | штатная установка APK из релиза |
| `fdroid` (фолбэк) | установка из F-Droid-клиента; `adb shell dumpsys package com.leadaxe.lxbox \| grep installerPackageName` |
| `play` | internal-трек; `adb shell pm list packages -i \| grep lxbox` → `com.android.vending` |
| define-ветка | `flutter run --dart-define=LXBOX_DISTRIBUTION=play` + Debug API `POST /action/check-updates` |
| «только на старте» | при работающем приложении дёрнуть `/action/check-updates` с новой версией — снек **не** появляется; `force-stop` + запуск → появляется |
| Later / Ignore | «Later» → force-stop → снова показан; «Ignore» → force-stop → молчит |
| без GMS | AOSP-эмулятор — клик по телу открывает браузер, не крашится |
