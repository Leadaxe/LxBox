...
которая закрыта как **won't-fix** (диагностика завершена, фикс `require()`
осознанно не делаем — цена > выгоды для не подтверждённого в проде окна).
...
- Поэтому `require()`-обёртка была бы отдельным улучшением для `onLost`-окна —
  **но §122 закрыта won't-fix**: окно в проде не подтверждено, `error()` не краш,
  цена `require()` (suspend-перестановка + таймаут) > выгоды. Детали — §122.
