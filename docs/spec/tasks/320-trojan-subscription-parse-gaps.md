...
**Тип:** bugfix (импорт/конвертация) · **Статус:** ✅ DEVICE-VERIFIED 30.07.2026 (v2.18.2) · **Размер:** M · **Область:** parser (URI + JSON) + tls/transport-модель

> Проверка на CPH2411 (сборка 2.18.0-dev.6, ядро `lx.17-rc.3`): подписка из
> 105 узлов переимпортирована новым парсером — в конфиге **0 ech-блоков**,
> туннель стартует (`last_start_error` пуст), 54 живых замера. A/B по ECH и
> сверка с базой NekoBox — см. Проблему 3.
