...
**Статус:** Реализовано (A+B); A device-verified (dev.67), B — скриншот впереди
...
## ✅ Device-verified (часть A, dev.67 build 2833, Android 15, 2026-06-26)

`/profiler/live?seconds=120`: 49 успешных unattributed `dnsResolve`, 0 `dnsFail`,
0 TCP/UDP без владельца → `unattributed_count: 0`, `banner_active: false`.
До §177-A эти 49 (порог 5) зажгли бы баннер; теперь он их игнорирует как норму.
Попутно подтверждён §176 — обе TCP-фазы (`tcpOpen: 49` + `tcpClose: 44`).
