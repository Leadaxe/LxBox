...
- [x] Checkbox in the wizard, default off; off = current plain WARP (byte-for-byte, short URI path).
- [x] On → the WARP node has 1.5-preset AWG fields in the endpoint-JSON; i1 is generated by the chosen template (WG/SIP), differs across two registrations. *(`warp_obfuscation_test.dart`, `awg_junk_test.dart`)*
- [x] s1=s2=0, h1..h4=1,2,3,4 → the handshake stays a valid WG. *(preset test)* — Cloudflare accept / traffic flow = **device-smoke pending**.
- [x] i1 (~1700b) does not break node-add (via `.conf`/`parseWireguardIni`); `reserved` (WARP client_id) reaches the peer/endpoint-JSON. *(round-trip test)*
- [x] Tests: preset is applied; i1 is random (≠ between calls); off = awg==null. *(13 tests)*
- [ ] Device-smoke against real DPI (or at least that the node connects with AWG fields). **PENDING — на телефоне.**

## Implementation (2026-06-15)

| Spec item | Code |
|---|---|
| Junk generator (2 templates, randomized) | `services/warp/awg_junk.dart` — `JunkTemplate{wgTraffic,sipTraffic}`, `generateJunkI1` |
| Pseudo user/host for SIP | `services/util/pseudo_gen.dart` ([§127](127-pseudo-name-domain-generator.md)) |
| 1.5 preset + i1 wiring | `WarpClient.amnezia15Preset()` / `buildAmnezia15Awg()`; `register(obfuscate, template)` |
| `WarpAccount.awg` + persist + `.conf` | `warp_account.dart` — field, toJson/fromJson, `toWireguardConf()`, `copyWith(clearAwg)` |
| `reserved`/`client_id` in `[Peer]` | `parser/ini_parser.dart` — reads `Reserved`/`ClientId`, carries via `reserved=` query |
| `.conf` node-add (tag kept WARP/WARP+) | `subscription_controller.dart` — `_addWarpObfuscated`, `_syncWarpObfuscation` |
| Wizard checkbox + template radio | `screens/warp_wizard_screen.dart` |

**Note on `s1=s2=0`:** emitted explicitly (preset carries them) so the config is 1:1 with the Amnezia 1.5 generator output; sing-box treats `0` as "no prefix" → handshake stays plain WG.
