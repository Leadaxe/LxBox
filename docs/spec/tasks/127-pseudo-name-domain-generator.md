...
- **`genWord()`** — произносимое слоговое слово (6–11 букв), выглядит как реальное имя.
...
- Блок `(C|ONSET)+V` повторяется **2–3** раза → длина 6–11 букв.
...
- [ ] `word()` — только `[a-z]`, длина 6–11, начинается с согласной, произносимо (нет `q/x/y`, нет онсета `lk` в начале).
- [ ] `user()` — ~30% содержат один `_`; обе части валидны как `word()`.
- [ ] `host()` — распределение ~30/30/30/10; IP всегда публичный (прогнать 1000× — ни одного зарезервированного).
- [ ] Два последовательных вызова дают разные значения (Random.secure, не seeded).
- [ ] Нет захардкоженных RFC-строк (`bob@biloxi.com` и т.п.).
