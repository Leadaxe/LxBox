...
**Обратная совместимость:** остальные пресеты (`ru-direct`, `fakeip`, …) СОХРАНЯЮТ плоские
`label`/`description`/`default`. Парсер `SelectableRule.fromJson` читает `json['ui']?['label']
?? json['label']` (объект побеждает, плоское — fallback). Не заставляет мигрировать все
пресеты сразу.
