...
Обязательна, по уроку §372 (гипотеза о поведении TV-прошивки там оказалась
неверной, реальность вскрыл только logcat):

- CPH2411 — основной путь SAF.
- Эмулятор Android TV API 31 — что `CREATE_DOCUMENT` действительно
  перехватывается заглушкой и что fallback в Downloads на нём работает.

До прогона на устройствах — помечать `DEVICE-PENDING`.
