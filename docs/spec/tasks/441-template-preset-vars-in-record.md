...
| Поле | Значение |
|------|----------|
| Статус | Реализовано в ветке `wave-441-vars`. Контракт 1.0.2 и кейс корпуса `v10_dns_template_vars` у лаунчера ещё не выложены: раннер готов читать фикстуру кейса, синк контракта и снятие `_pendingPortableVars` (L7) — после выкладки |
| Дата | 2026-09-15 |
| Норма | SPEC 128 лаунчера «Значения переменных живут в записи» (`SPECS/128-F-N-DNS_TEMPLATE_VARS`, контракт 1.0.2), §11 L1–L9 |
...
| | `post_steps/heal_detour_dropped_dns.dart` | Одно место политики ссылок на выпавший сервер: DNS-правила → `action: reject` (поля маршрута сняты); `dns.final`, `route.default_domain_resolver`, `servers[].domain_resolver` — замена политикой §419 (умолчание шаблона, иначе первый пригодный), без записи в хранение |
| | `resolveTemplateDnsServerBody` | `@name`, не объявленный сервером, не подставляется: ключ выпадает, warning сборки |
| L9 | заголовок `dns_backup.dart` | Переписан: `vars` и `description` сервера едут, нормы SPEC 128 |
...
- **Замены `dns.final`/резолверов не персистятся** для выпавшего сервера (в
  отличие от §419): выбор пользователя цел, вернётся Направление — вернётся
  сервер. Форму «до прямого сервера не должно дойти ничего» лаунчер
  доопределит в SPEC 128; менять одну функцию `healDetourDroppedDnsRefs`.

## Ссылки на Направление в значениях и Л5 (волна 441b)

SPEC 128 §6, D-113/D-114: значение переменной типа `outbound` в записи —
