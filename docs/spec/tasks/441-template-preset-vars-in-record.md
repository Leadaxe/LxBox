...
- **Л5** (`vars` у `user`/`preset` DNS-записи как `backup_unknown_field`) не
  делалось: кодек такой ключ игнорирует.
...
- Удаление и выключение Направления (`_healDirectionRefs`) переводят на
  `vpn-1` цели правил, но не `vars.outbound` template-сервера DNS: такой
  сервер теперь выпадает на сборке (Н10), а не уходит напрямую. Лечить ли
  ссылку в хранении (SPEC 128 §6) — отдельная задача; переименования
  Направлений у LxBox нет.
