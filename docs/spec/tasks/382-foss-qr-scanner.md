...
**Настройки распознавания — правились по фидбэку с устройства.** Первая
сборка ощущалась заметно хуже ML Kit («ищет в маленьком кусочке экрана, не
сразу находит»). Причина была не в ZXing, а в дефолтах виджета, которые я
не переопределил:

| Параметр | Дефолт | Стало | Почему |
|---|---|---|---|
| `cropPercent` | 0.5 | **0.9** | виджет отдаёт декодеру ровно тот квадрат, что рисует рамкой (`min(w,h) × cropPercent`, `reader_widget.dart:474`) — 0.5 это лишь ¼ площади кадра |
| `tryHarder` | false | **true** | быстрый проход сдаётся на блике/наклоне; сканируем один код по требованию, тщательность важнее батареи |
| `tryDownscale` | false | **true** | крупный код с близкого расстояния иначе не берётся |
| `scanDelay` | 1000 мс | **300 мс** | это пауза ПОСЛЕ неудачи, а не такт между кадрами: при дефолте попытка раз в секунду, все кадры между ними выбрасываются |

**Индикатор попыток.** Кнопки съёмки нет — попытки идут сами, и юзеру
неоткуда понять, работает сканер или завис («когда оно вообще пытается,
непонятно»). Точка рядом с подсказкой моргает на каждой неудачной попытке
через `onScanFailure`; неподвижная точка = сканер встал.
...
| 5 | Ни одного упоминания ML Kit / `libbarhopper` в APK | ✅ 0 совпадений |
| 6 | Размер APK (arm64) записан | ✅ 38.99 МБ, см. ниже |
| 7 | APK устанавливается и запускается на эмуляторе | ✅ pid жив, крашей нет |
| 8 | Экран сканера открывается, превью камеры живое | ✅ скриншот, рамка + фонарик |
...
**Размер APK (п. 6).** Текущая сборка arm64 — **38.99 МБ**. Прямой дельты
к §375 (38.38 МБ) вывести нельзя: тот замер делался с `--split-per-abi`,
который §379 убрал, и с другой версией ядра — `libbox.so` доминирует
(74.89 МБ несжатых) и с тех пор менялся. Сопоставимо сравнивать можно
только вклад самого сканера по нативным библиотекам:

| | ML Kit (§375) | ZXing-C++ (§382) |
|---|---|---|
| нативная библиотека | `libbarhopper_v3.so` 4.95 МБ | `libflutter_zxing.so` **1.81 МБ** |
| ML-модели | 3 × `.tflite`, 0.88 МБ | нет |
| Итого несжатых | ~5.83 МБ | **1.81 МБ** |

То есть по декодеру выигрыш ~4 МБ несжатых; общий размер APK при этом
почти не сдвинулся, потому что `camera`/CameraX добавляет свою обвязку
(`libimage_processing_util_jni.so`, `libsurface_util_jni.so` и dex).

**Грабля сборки.** `flutter_zxing` требует **NDK 27.0.12077973** — при
первой сборке он доустанавливается автоматически. §380 сводит проект к
NDK r28c; расхождение нужно проверить перед прогоном на buildserver, иначе
рецепт F-Droid потянет вторую версию NDK.
