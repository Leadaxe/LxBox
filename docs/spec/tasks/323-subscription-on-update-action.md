...
- Реакция на **ручной** refresh (⟳). Ручной refresh — явное действие юзера, он
  и так видит плашку и жмёт Apply сам. Действие применяется только к
  авто-триггерам.
...
5. Ручной ⟳ в режиме `reload` туннель НЕ рвёт.
