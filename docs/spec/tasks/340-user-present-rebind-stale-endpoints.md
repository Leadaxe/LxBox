...
| | |
|---|---|
| Статус | Вошло в v2.19.2 (AAR `v1.14.0-lx.19-rc.3`). На устройстве прогнано — регрессий нет; **целевой сценарий (сон → разблокировка → пинг с первой попытки) на момент релиза ещё проверялся** |
| Дата | 2026-08-02 |
