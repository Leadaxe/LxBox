...
## §136fix — padding до полноразмерного QUIC Initial (2026-06-16)

**Баг:** боевой генератор давал i1 = **92 байта** (порт quic.js с `padto=0`).
Декодирование рабочего i1 у юзера (`WARPv1_76.conf`): **1250 байт** (QUIC
length-поле 1232, DCID=8). RFC 9000 требует Initial ≥1200 — короткий «Initial»
92б в природе не существует → DPI не примет за настоящий QUIC.

**Корень:** `quic.js` `generate()` зовёт `quicInitial(..., padto=0)` — оригинал
тоже короткий. Рабочие тестовые узлы для юзера были 1250б (отдельный
Python-скрипт с padding), Dart-порт quic.js padding потерял.

**Фикс** ([quic_i1.dart](../../../app/lib/services/warp/quic_i1.dart) `generate`):
паддим payload QUIC PADDING-фреймами (нули) до `_minPayloadLen=1200` → пакет
~1235б. `_padCut` расширяет предпоследний `<b>`-сегмент на padding → нули
попадают в вывод. **`<r>`-сегменты СОХРАНЕНЫ** (beacon-защита: ядро рандомит их
per-packet) — паддинг идёт в `<b>`, не вместо `<r>`. DCID поднят 1→8 (как в
рабочем i1). Берём лучшее с обеих осей: уникальность (`<r>`) И полноразмерность.

Тест: `§136fix — пакет ≥1200б … <r> цел` (level 0-4).
