...
**Статус:** Debug-половина (§2.0-2.3) реализована и device-verified 27.07.2026;
пользовательская половина (§2.4-2.8) реализована 27.07.2026 — DEVICE-PENDING
...
**Пользовательская половина — DEVICE-PENDING.** Проверить, когда будет живой
краш (или подложив файл в `crash_reports/` через `run-as`):

1. Diagnostics → «Crash reports»: список, дата-время локальные, тап отдаёт
   файл в системный share.
2. Крашей нет → «No crash reports» (а не пустой экран).
3. Плашка на главном после краша; повторный запуск — молчит; более свежий
   файл — снова показывается.
4. `Share dump` → в JSON есть `crash_archive` с телами файлов.
5. 11+ файлов в архиве → после рестарта приложения остаётся 10.
