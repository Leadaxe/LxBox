...
| Поле | Значение |
|------|----------|
| Статус | **Done** (2026-06-22) — реализовано: allowlist в `replaceRaw`, экспорт расфильтрован, droppedKeys→applog+снэкбар, DENY/миграции удалены, seed распутан, ping_options strip. `flutter analyze` чисто, все 1186 тестов прошли (+ новые §159 allowlist-тесты). На железе не проверялось (телефон не подключён). |
| Дата | 2026-06-22 |
...
- [ ] **`debug_token` в импорте.** Это валидный var (allowlist B2), но перезапись
      через import-вход = lockout Debug API (`_varBlocklist` его уже защищает на
      `PUT /settings/vars/{key}`, но `replaceRaw` идёт мимо). Решить: импорт
      сохраняет текущий `debug_token` / отбрасывает его из входа / пускает как
      есть. Скорее всего — **сохранять текущий** (как `_varBlocklist`).
- [ ] **Разведение экспорта и импорта.** Сейчас оба зовут общий
      `_filterStorageForImport`. После п.3 экспорт перестаёт фильтровать — убедиться,
      что новый «нефильтрованный» экспорт не ломает чтение существующих бэкапов
      (формат wire-объекта не меняется, меняется только полнота `storage`-блока).
