...
| Поле | Значение |
|------|----------|
| Статус | **⚠️ SUPERSEDED (18.07.2026)** — AWG-over-WG разрешён: guard в ядре снят (SPEC 007, ядро lx.11 — см. KERNEL.md), app-ограничение §130 удалено (коммит b1ffc66). Ниже — история реализации (была Done в v2.3.1 на ядре v1.13.13-lx.9). |
| Verification | `flutter analyze --no-pub` чисто; release arm64 build пройден; установлено на тест-телефон, UI подтверждён вручную 2026-06-16 (подпись «AmneziaWG (wireguard)», detour без WG-целей, сброс невалидного detour). |
