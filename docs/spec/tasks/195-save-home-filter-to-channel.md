...
от `×` (clear) добавить кнопку сохранения (💾). Видна когда поле непустое. Клик →
диалог выбора, **куда** записать текущий regex в **активный канал**
...
После записи — `updateChannel` в storage + rebuild конфига (как любой channel-edit).
