...
| Поле | Значение |
|------|----------|
| Статус | Draft — диагностика завершена, фикс ГЕЙТНУТ на замер (см. «Решение») |
...
## Решение — `require()` с таймаутом, ГЕЙТНУТО на замер

### Гейт (сделать ПЕРВЫМ)

Прежде чем писать код — **подтвердить, что баг реально стреляет**. Безусловный лог
`LxBoxNet` уже есть (§119, TD-119-1). Добавить временный счётчик/лог на ветку
`error("missing default interface")` в `LocalResolver` и снять с прода/репортов:

- **Не стреляет** (или единичные на старте) → чинить нечего, апстрим прав, что
  читает поле синхронно. Закрыть §122 как won't-fix.
- **Стреляет на `onLost`** (повторяется при смене сети) → реализовать require().

### Концепт фикса (если гейт пройден)
