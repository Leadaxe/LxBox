...
| Событие | Действие |
|---|---|
| `_applyGroups` (push-стрим / SPEC015-pull / reloadProxies), снапшота нет | lazy-fetch `_ensureRunningConfig()` (guard от параллельных; null-ответ не ретраим агрессивно — следующий groups-push попробует снова) |
| любой сброс снапшота | `_invalidateRunningConfig()` — bump epoch'а + сброс **одним движением** (см. epoch-гейт ниже) |
| Переход в `connected` | сброс в null → свежая сессия перезахватит (закрывает reload и реконнект) |
| Переход в down (disconnected/revoked / force-stop timeout / heartbeat-dead) | сброс в null — те же copyWith, что чистят `ccGroups` |
| `reloadVpn()` | явный сброс до native-вызова (ядро перезахватит на новом старте) |
| Холодный старт приложения поверх живого туннеля | CC приаттачился → groups → lazy-fetch → **дыра §309 закрыта**: правда от ядра, файл не при чём |
