...
TCP 443 вовсе (не 4xx, не RST — тишина до таймаута). Регистрация падает
через 15 с с «network error», узел не добавляется. Через VPN тот же хост
отвечает нормально.
...
|---|---|
| исключение клиента (SocketException, таймаут `_timeout`) | лог `WARP: API host … unreachable`, следующий хост |
...
MASQUE-edge; edge `162.159.198.2:443` его принимает. Он уже первый и
