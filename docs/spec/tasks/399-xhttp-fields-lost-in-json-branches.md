...
> **СТАТУС: РЕАЛИЗОВАНО** 18.08.2026. Критерии 1–8 закрыты тестами
> (`test/parser/xhttp_test.dart`, группа §399), `flutter analyze` + 3058 тестов
> зелёные. **DEVICE-PENDING**: живой Xray-JSON подписки с XHTTP-узлом на
> `packet-up`+`GET` под рукой нет — узлы из §1 принадлежат чужому проекту.
