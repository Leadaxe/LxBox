...
## Нормы контракта 1.1.68–1.1.70 (SPEC 143)

Паритет с лаунчером по языку шаблона — задача
[555](../../tasks/555-template-lang-spec143-parity.md):

- ветка `#if` элемента массива, если она массив, вливается в родителя на один
  уровень (TEMPLATE_LANG §4.4, N13); вложение — двойными скобками `[[..]]`;
- `options` ортогональны `type`, `options_open` разрешает своё значение,
  каст только по `type` (§2.1, §2.2, N14);
- `@runtime.platform/arch/target` на телефоне объявлены со значением null →
  Dropped ключа/элемента, в предикате `false`; прочее `runtime.<x>` —
  `template_var_undeclared {name}` и плейсхолдер (§7.2, N15);
- предупреждения движка — код реестра с параметрами, дедуп по (код,
  параметры); сборка собирает их в `BuildResult.templateWarnings`, строки
  идут первыми в отчёте сборки и сохранение не блокируют;
- гейты фрагментов пресета и шаблонных DNS-серверов дают
  `template_fragment_dropped {owner, kind, reason}`.
