...
| Поле | Значение |
|---|---|
| **Статус** | Реализовано (device-verified; ядро `v1.14.0-lx.1-rc.4`, главный экран/Stats/Conns/профайлер на CommandClient, Clash API выпилен) |
| **Дата** | 2026-06-24 |
| **Целевое ядро** | sing-box-lx `v1.14.0-lx.1-rc.4` (мажор-связка `v1.14.0-lx.1`; **base upstream sing-box `v1.14.0-alpha.33`** — merge `514a9e74`, отсюда DNS-rework/native API service/closed-история, на которые опирается §3/§8; + `with_awg`/`with_xhttp`/`with_lx_command`). **Два AAR:** основной (**SDK23+**) и legacy (**SDK21**). `Libbox.version()` → `1.14.0-lx.1-rc.4` (полный тег с суффиксом, `tag.go:9-21`; БЕЗ `v`). **Version-gate сравнивать по ПРЕФИКСУ `1.14.0-lx.1`, не точным равенством** — иначе rc.N не пройдут. `rc.4` добавил unary-pull `getGroups` (SPEC 015). SPEC 014/015: реализовано. |
...
**1. Безопасность и гигиена конфига.** Пока проектный `.so` линкует Clash API server, блок `experimental.clash_api` открывает живой TCP-порт на `127.0.0.1`, доступный любому приложению на устройстве; единственная защита — `secret`. Это attack surface. После rc.2 (server вырезан, см. §1a) инъекция `clash_api` даёт **ФАТАЛЬНЫЙ ОТКАЗ СТАРТА ядра** — НЕ no-op (проверено на железе device-smoke 2026-06-24): `create clash-server: clash api is not included in this build, rebuild with -tags with_clash_api`. Ядро видит блок в конфиге, пытается поднять clash-сервер, которого в бинаре нет → VPN не стартует. В обоих состояниях `.so` инъекция `clash_api` нежелательна: либо дыра (server есть), либо **фатальный отказ** (server вырезан). Отказ от неё закрывает оба случая.
...
**Следствие для §122.** Перед/в рамках миграции проектный AAR синхронизировать с rc.1 (server-less). **Это БЛОКЕР запуска на rc.2:** AAR без with_clash_api + конфиг с `clash_api` = ядро не стартует. Поэтому выпил инъекции `clash_api` (§5.3) + defensive-strip старых конфигов (§6.2, `ConfigManager.stripClashApi`) обязательны ВМЕСТЕ с подкладкой rc.2 AAR, не отдельным шагом.
...
|---|---|---|---|---|
| **Group URLTest** (`runGroupUrltest`) | `ping_orchestration.dart:169` | `GET /group/{tag}/delay` → `groupDelay()` (`clash_api_client.dart:143`) | штатный `urlTest(groupTag)` (императив); результат для членов группы через `writeGroups`, для node-list — через `writeOutbounds` (плоский, §4.3) | паритет 1:1 (нулевой дифф в ядре — не трогать). ⚠️ Реализация §122 от этой строки отклонилась: был подключён per-node `urlTestOutbound(groupTag)` — один замер СКВОЗЬ текущий выбор группы, БЕЗ перебора членов и переселекта (мёртвый выбор оставался до interval-тика ядра). Исправлено в §308: `ccUrlTestGroup` → `CommandClient.urlTest(groupTag)`, fire-and-forget, URL из конфига группы |
...
Мотив (см. §1a): пока проектный `.so` линкует Clash server, чужой `clash_api` в импортированном конфиге **откроет живой TCP-порт** на 127.0.0.1 (наша гигиена обнулится через чужой конфиг). После rc.2 (server вырезан) — **фатальный отказ старта** (ядро не может создать clash-server). Вырезание ОБЯЗАТЕЛЬНО: в импорт-пайплайне (§159) + defensive-strip в `ConfigManager.load/save` (§6.2) для старых сохранённых конфигов.
