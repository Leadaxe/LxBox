...
Решение отложено, вопрос лаунчеру задан 14.09 (что делают они). Варианты:
A — при удалении узла удалять каталог, при переименовании переносить; для
этого хранить последний эмитированный тег (финальный тег знает только
сборка: префикс папки, уникализация `allocateTag`). B — при сборке убирать
каталоги, чьего тега нет среди эмитированных узлов Tailscale (список у
сборки есть), и предупреждать при переименовании. Пока каталоги живут до
