...
- Имя (или host из URL для подписок без `profile-title`)
- Subtitle: для `SubscriptionServers` — `124 nodes · 🔄 24h · 🕐 3h ago · (2 fails)`; для `UserServer` — `<PROTOCOL> server` (например `WIREGUARD server`, `VLESS server`).
- Статус: загружена / ошибка / не обновлялась (см. spec 027 — `lastUpdateStatus`)
...
**Status:** Реализовано (1.3.x)

Триггеры (оба идут через единый `_pasteFromClipboard`):
- Кнопка Paste в popup menu сверху
- **Тап `+` при пустом поле ввода** (1.3.1) — экономит шаг

Диалог подтверждения с автоопределением типа содержимого:
