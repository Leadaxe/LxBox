...
| SubscriptionDetailScreen (список нод подписки) | **Показаны с ⚙** | Показаны с ⚙ |
| Statistics (outbound cards) | Показаны (если есть трафик) | Показаны |
| Connections list | Показаны (в chain) | Показаны |

**SubscriptionDetailScreen**: jump серверы **всегда видны** в списке нод подписки с префиксом `⚙`. Скрывать их здесь не нужно — пользователь должен видеть полный состав подписки, включая jump серверы. Настройка `show_jump_servers` на этот экран не влияет.
