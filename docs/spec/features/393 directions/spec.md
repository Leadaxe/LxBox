...
- **Полевое правило раскладки (наблюдение мобилы, 2 конфигурации)**:
  UDP-туннели (WG/MASQUE/QUIC) — входными хопами; UDP-хоп ПОЗАДИ
  TCP-хопа (VLESS и т.п.) умирает, если тот не проксирует UDP
  (`MASQUE via VLESS: deadline` стабильно). Кандидат в предупреждение формы:
  «позиция N — UDP-протокол за хопом, возможно не проксирующим UDP».
