...
| 417 | [`417 workspaces/`](417%20workspaces/) | Workspaces: именованные копии состояния (настройки + кэш подписок + .srs); Load = автосохранение текущего → копия → перечитать без рестарта → пересборка → VPN; Save as; без переезда файлов | ТЗ |
