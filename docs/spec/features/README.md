...
| 439 | [`439 storage-contract-1-0/`](439%20storage-contract-1-0/) | Хранение `lxbox_settings.json` в форме записей контракта 1.0 (`sources[]`, `rules[]`, `dns{}`), миграция при загрузке с копией `.v0.bak`, экспорт LX Backup — срез хранения | Спека |
