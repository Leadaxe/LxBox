...
skip+warning; backward-compat (нет `dns` → старое поведение).
...
- Выбранный сервер удалён → правило с висящим `serverTag`: skip DNS-rule +
  warning, broken-индикатор (как broken-preset).
