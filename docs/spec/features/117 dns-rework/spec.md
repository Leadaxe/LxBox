...
3. **Исчезнувший реф — тихо, без ошибок**: канал пропал → detour исчез;
   DNS-сервер пропал → DNS из правила **тихо пропадает** (без warning /
   broken-индикатора). *(Переопределяет ранний edge «skip+warning».)*
