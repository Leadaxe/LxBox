...
| Поле | Значение |
|------|----------|
| Статус | 🚫 Закрыто (не актуально) — оставшиеся roadmap-пункты сняты |

> **Закрыто.** Базовая часть (описанная ниже в «Что уже защищено») в продакшене.
> Оставшиеся roadmap-пункты (Android Keystore encrypted storage, маскировка
> credentials/URL в логах, certificate pinning, ProGuard/R8) признаны неактуальными
> для угрозовой модели L×Box (dev-tool / self-hosted) и с плана сняты. Anti-DPI-часть
> живёт в профильных фичах (WARP/AWG/uTLS/fragment), не в этой зонтичной спеке.
