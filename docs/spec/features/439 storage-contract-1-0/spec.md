...
| 8 | `hops`, `detour` в модели — финальные теги конфига; в 1.0 — `{folder_id?, tag}` | BACKUP §4: «LxBox адресует хоп тегом и пустой `folder_id` читает как корень — это законная запись». Хранение пишет `{tag: <финальный тег>}`, модель не меняется, `_LinkIndex` остаётся на экспорте (Л1) |
...
| 13 | `CustomRule.toJson()` — не только хранение: сравнение в `isDirty` (`custom_rule_edit/edit_controller.dart`), JSON-вкладка редактора (`tabs/view_tab.dart`), файл правил (`rule_transfer.dart`), носитель патча `PATCH /rules/{id}` (ключи `domainSuffixes`…) | всё на форму записей; файл правил → `format: 2`, `format: 1` читает legacy-читатель; `PATCH /rules/{id}` патчит модель через `copyWith` по виду; снаружи Debug API (`serializeCustomRule`, snake_case) не меняется |
...
- **В1 — да.** В 2.23.3 истина узла — текст исходника. «Тело как истина» с
  ...
  узла.
- **В2 — пояснение запрошено**, ответ ещё не получен; до ответа миграция по
  рекомендации (деление на правила `имя`, `имя #2`).
...
- **Л1 — нет, пока.** Ссылка без `folder_id` у лаунчера ищется только среди
  корневых узлов; это дефект лаунчера против BACKUP §4, чинится патчем после
  1.6.0. До фикса `_LinkIndex` на экспорте 2.23.3 **остаётся**.
