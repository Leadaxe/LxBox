...
| Поле | Значение |
|------|----------|
| Статус | Реализовано и проверено на AVD: этапы 0, A, B, C, трек N, контракт 1.0.1 (Л2 объявлены, D-115…D-117), фиксы волны E влиты в develop (`1590ab3b`). Полный набор тестов 4619 passed / 0 failed, корпус 30/30. Волна E (`230c3354`) нашла баги, E2 (`753d10e4`, чистый повтор миграции с формы 2.23.2) их подтвердила исправленными, финальная сборка `1590ab3b` — конфиг и импорт 1.0 (§5.3). Отклонения от плана — §6.6. Открыто: переносимые vars DNS-маршрута (у LxBox в `dns.servers[].vars`, у лаунчера в общих `vars`) — ждёт решения владельца |
