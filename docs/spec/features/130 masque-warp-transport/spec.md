...
**Статус:** ✅ Реализовано и протестировано — в релизе (v2.9.0). Полный цикл h3+h2:
Dart-регистрация WARP → эмит `type: masque` → живой прогон с ядром пройден.
**Ядро:** sing-box-lx SPEC 021 (`type: masque`, ветка `lx-spec021-masque`, h3+h2 device-verified
2026-07-02). Ядро реализовано; live-тест с реальным ключевым материалом из Dart-регистрации пройден.
...
> ⚠️ **Схема конфига устарела с ядра `v1.14.0-lx.25-rc.4`.** Всё, что ниже
> описано как `network` / плоский `sni`, теперь эмитится как `transport` и
> вложенный `tls{}`; в `masque_account` транспорт больше не хранится вовсе.
...
## Endpoint-пул (§305, device-verified)

`server`/`server_port` берутся из ответа регистрации, но узел можно поставить и на другой
адрес блока — вручную (поля **Endpoint IP** / **Port** в визарде) или посевом генератора.
Какие адреса реально живые — зафиксировано в `app/assets/warp_endpoints.json`:

```jsonc
"masque": {
  "v4_cidr":    ["162.159.198.0/24", "162.159.199.0/24"],  // h2 живёт по ВСЕМУ блоку
  "h3_v4_cidr": ["162.159.198.1/32", "162.159.198.2/32",   // h3 — ТОЛЬКО эти 4 адреса
                 "162.159.199.1/32", "162.159.199.2/32"],
  "ports_h3": [443, 500, 1701, 4500, 4443, 8443, 8095],    // рабочие все 7 …
  "ports_h2": [443, 500, 1701, 4500, 4443, 8443, 8095],    // … для обоих транспортов
  "sni_pool": [...]
}
```

- **h3 (QUIC)** поднимается **только на 4 адресах**. На остальных IP блока — нет; рандом по
  `/24` давал ~1% попаданий (отсюда «генератор плодит мёртвые h3»).
- **h2 (HTTP/2)** живёт по всему блоку — десятки адресов.
- **SNI на h3 не влияет** — коннект встаёт даже с несуществующим доменом (но пустой SNI
  оставлять не стоит: дефолт ядра `consumer-masque.cloudflareclient.com` палевен для DPI,
  визард всегда проставляет домен из `sni_pool`).
- Блоки `162.159.197.0/24` и `.192.0/24` для MASQUE мертвы, из пула убраны.

Выбор источника в коде — `ScanPool.masqueV4CidrFor(net)` / `masquePortsFor(net)`.
**Как корректно мерить живость** (наивный probe врёт про QUIC) — [DIAGNOSTICS.md](../../../DIAGNOSTICS.md)
и [spec/tasks/305](../../tasks/305-masque-endpoint-h2-pool-and-override.md).
