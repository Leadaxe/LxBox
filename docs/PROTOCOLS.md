...
### Уровни awg / awg1.5 / awg2 (§148)

Subtitle узла и variant-фильтр (§102/§103) различают уровень обфускации структурно — по наличию полей в конфиге ([`config_node.dart`](../app/lib/models/config_node.dart)). База определяется по старшему присутствующему маркеру (приоритет сверху вниз):

| Лейбл | Условие |
|-------|---------|
| `awg2` | transport-padding `s3`/`s4` **или** CPS-приманки `i2`–`i5` |
...
Суффикс `+` (`awg+` / `awg1.5+` / `awg2+`) дописывается поверх любой базы при наличии masquerade-sugar `ip`/`id`/`ib` (§143). Эти поля взаимоисключающи с явным `i1` на уровне ядра (ядро 009 само генерит `i1` из них и отвергает оба сразу), но лейбл считается по сырому JSON до валидации — потому суффикс проверяется независимо от базы.
