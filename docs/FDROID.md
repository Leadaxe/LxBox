...
8. **Пин на генерируемую ветку может отвалиться.** `cronet-go` держит собранные
...
Режим **не включён**: `Binaries` и `AllowedAPKSigningKeys` в метаданных нет, APK
в каталоге подписывает F-Droid своим ключом. Цепочки обновлений с GitHub и из
каталога независимы — перейти с одной на другую можно только через удаление.

Решение осознанное: включённый режим обязывает каждый релиз сходиться побитово,
иначе версия не выйдет в каталог.

Само совпадение достигнуто и проверено на v2.20.4, все три ABI: **455 файлов,
ноль расхождений**. Если рецензент попросит включить режим — всё готово.
...
— 55-70 минут, три — **2 часа 54 минуты** (замер на v2.20.4).
