...
> **Historical archive — not maintained.** Документ заморожен на v1.9.0 / кануне
> v2.0.0 и не отражает текущее состояние. Актуальная версия, счётчики и roadmap —
> в [`CHANGELOG.md`](../CHANGELOG.md) и
> [`docs/spec/features/README.md`](spec/features/README.md).
>
> Это единственный документ верхнего уровня, оставленный на русском (§394):
> переводить замороженную хронику смысла нет. Остальная документация — на
> английском, пользовательские документы ведутся парами `X.ru.md`.
