...
| gRPC `serviceName` in the Xray absolute-path form `/<service>/Tun` | leading `/` and the trailing `/Tun` stripped (`/abcde/Tun` → `abcde`); a `|multi` tail on the stream segment ignored | silent | `transport.dart:86-93`, shared rule `body_sanitizer.dart:711` `normalizeGrpcServiceName` | in Xray the leading `/` means a different notation, where the last segment names the **stream**, not the service. sing-box never configures the stream (always `Tun`), so the wire path is preserved. Same rule on the Xray-JSON branch — one node must not read differently by input. Core limitation: an inner `/` still goes on the wire as `%2F`, so a multi-segment name will not match the server | §464 (issue #130, registry `normalize: grpc_service_name`) |
