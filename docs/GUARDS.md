...
| Hysteria2 `up_mbps` / `down_mbps` in the URI | **read as aliases** of `upmbps` / `downmbps` (emission still writes `upmbps`) | silent | `hysteria2_parser.dart:133-134` | W2d made both spellings registry aliases (`hysteria2.json uri.query.upmbps.aliases`) and the launcher now accepts both. Until §464 a link carrying `up_mbps=` lost its bandwidth silently | §464 (contract §24.7 item 4) |
| Hysteria2 `obfs-min-packet-size` / `obfs-max-packet-size` with `obfs` other than `gecko` | field dropped | `RegistryWarning` `field_requires` | `hysteria2_parser.dart:136-159` | the sizes are gecko-only (registry `requires` + `equals`). The emitter already skipped them for salamander, but silently — the same node arriving as a body got a code from the sanitiser | §464 (contract §24.7) |
| Hysteria2 `fp` / `pbk`+`sid` in the URI | **read, but only to name the loss**: neither reaches the model — the emit strips both blocks anyway (`toSingboxForQuic`) | `RegistryWarning(tls_not_applicable_quic)`, severity `info`, one per block (`tls.utls`, `tls.reality`) | `hysteria2_parser.dart` (after the TLS block), rule from `parse_warnings.dart` `forbiddenTlsBlockWarnings` | uTLS and REALITY do not exist over QUIC (the core builds its TLS through `STDConfig()`, which neither provides), so the strip is right — but until 1.1.4 it was silent and the user never learned `fp=` had no effect. The code is set **by the parser, not the sanitiser**: the sanitiser of the parse pass looks at `emit()`, where the block is already gone. Which schemes and which code comes from the registry | §469 / contract 1.1.4 |
...
| hysteria2 `obfs` in JSON: type outside `{salamander, gecko}`, or a valid type with no password | obfs dropped whole — same funnel as the URI (`normalizeHysteria2Obfs`) | `UnknownObfsWarning` / `MissingObfsPasswordWarning` | `json_parsers.dart` `parseSingboxEntry`, case `hysteria2` | until §469 this path passed `null` for the accumulator and both codes were swallowed: the same node arriving as a link warned and arriving as a body said nothing, though the body it produced was identical. The launcher had the mirror-image defect (`sanitizeSingboxHysteria2Obfs`, fixed in `371448da`) | §358, §469 item 6 |
| hysteria2/tuic/masque body carrying `tls.utls` or `tls.reality` | blocks do not reach the config (the emit strips them) | `RegistryWarning(tls_not_applicable_quic)`, severity `info`, one per block; `value` is the block **as the body carried it** | `json_parsers.dart` `parseSingboxEntry`, rule from `parse_warnings.dart` `forbiddenTlsBlockWarnings` | the same registry rule as the URI path — a node must carry the same codes whichever way it arrived. MASQUE matters only here: its link format carries no `fp`/`pbk`, and `MasqueSpec` knows no such fields at all, so in a hand-written body the block vanished without a trace | §469 / contract 1.1.4 |
