...
| `key_share=` in any case / with spaces (`Hybrid`, ` classical `) | `trim().toLowerCase()`, then the enum | silent | *moved to the registry, §1.0* — the mapper (`mappers/common_parts.dart`) copies the value verbatim into `tls.reality.key_share`, and `tls.json` normalises it (`normalize: trim_lower`) before the enum; spec 472 steps 3 and 6 removed the hand-written `realityKeyShareFromQuery` from the link path | the core is case-sensitive, but the value comes from the subscription — a case difference is the source's intent, not garbage, and used to be lost silently | §459 (contract §24.2 item 7.12) |
