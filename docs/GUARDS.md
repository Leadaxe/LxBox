...
`trojan` (step 2), `vless` (step 3), `vmess` and `shadowsocks` (step 4) and
`hysteria2` and `tuic` (step 5, the scheme alias `hy2://` included) reach the model through
the unified pipeline — mapper → registry sanitiser → `parseSingboxEntry` — so
the value rules listed in §§1.1–1.5 below **no longer run for them**. The rules themselves did not disappear: the same judgement is
...
| AnyTLS / TUIC durations as bare numbers | `s` suffix appended | silent | `anytls_parser.dart:62-64`, `mappers/tuic_mapper.dart` (registry rule `heartbeat_bare_number`) | whole-config fatal otherwise | D-024 |
