...
| AWG `mtu` above 1280 (link / `.conf` / Amnezia export) | clamped to the registry ceiling (1280) | `awg_mtu_clamped` (warning), carrying the original value | `uri_utils.dart` `awgClampMtu` → `registry.dart` `awgMtuByRegistry`, code in `parse_warnings.dart` `awgMtuWarnings` | too high is a silent failure: handshake succeeds, data does not flow. The ceiling, the marker-field set and the default all come from `wireguard.body.fields.mtu.max_when` — no second copy of the rule in Dart | §473 (contract 1.1.5), was §097 |
...
| AWG `mtu` absent | default 1280, on every input including sing-box bodies | silent — a default is not a replacement | `registry.dart` `awgMtuByRegistry` (`default_when.when.any_set`) | AmneziaWG's own recommended client MTU and the IPv6 minimum | §473, was §097 |
