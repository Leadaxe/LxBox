...
| Xray `users[].security` (VMess) outside the core's enum / absent | the same funnel as the URI and sing-box JSON → `auto` | silent (AppLog only) | `json_parsers.dart` `_xrayVmessToSpec`, `normalizeVmessSecurity` | one funnel for all three inputs: the branch used to take the value verbatim | §459 (contract §24.2 item 7.11) |
| Xray VLESS `flow=xtls-rprx-vision-udp443` | flow → `xtls-rprx-vision`, `packet_encoding: xudp`; **the port is not touched** | silent | `json_parsers.dart` `_xrayVlessToSpec`, `_xrayIdentity` | the port is a property of the node. Rewriting it to 443 made a `…:8443` node unreachable — a bug both clients had | §459 (contract §24.2 item 7.4), was §321 |
...
| Xray gRPC `grpcSettings.serviceName` in the form `/<service>/Tun` | leading `/` and trailing `/Tun` stripped | silent (no warnings channel) | `json_parsers.dart:966-974`, shared rule `body_sanitizer.dart:711` | the same registry rule as the URI branch: the same node must not read differently by input | §464 (issue #130) |
