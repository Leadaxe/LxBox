...
- **Body для template/preset берётся из canonical** at render/build time → tag rename'ы из §039 (`direct_dns_resolver` → `google_udp`) теперь подхватываются automatically.
...
- **После tag rename из §039 (`direct_dns_resolver` → `google_udp`)** existing-юзеры не видели нового tag'а в DNS Final dropdown'е (storage XOR template). Auto-discovery теперь подтягивает новый tag, orphan cleanup убирает старый.
