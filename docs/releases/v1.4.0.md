...
## 🛠 Reconnect reliability overhaul

В v1.4.0 зафиксированы три связанные проблемы в VPN-lifecycle/status pipeline, которые проявлялись как «плашка *Config changed — restart VPN to apply* не уходит после reconnect», «Stop button иногда игнорируется», «отвалившийся tooltip». Единый ремонт, четыре коммита.

**1. Status-sink leak (root cause).** `BoxVpnClient.onStatusChanged` — getter, который на каждый вызов создавал новый `receiveBroadcastStream()`. Каждый такой вызов вызывал `EventChannel.onListen` на native, перезаписывая shared `statusSink`; следующий `onCancel` (при закрытии короткоживущей подписки типа `firstWhere` в reconnect) обнулял его. После первого reconnect'а основной listener в `HomeController._statusSub` становился зомби: Dart думает что подписан, native давно выбросил sink. Все transition events (Starting/Started/Stopping/Stopped) после этого терялись.

Fix: `late final _statusStream = ....asBroadcastStream()` — один underlying controller на весь lifecycle клиента, native onListen вызывается ровно один раз. Множественные Dart-listener'ы (основной + `firstWhere` в reconnect) работают параллельно, не перехватывая друг друга.

**2. Blocking `stopVPN` на native.** Method channel возвращал `true` моментально (после отправки broadcast'а), но native выполнял cleanup libbox-ресурсов ещё ~50-80мс. `await _vpn.stopVPN()` → `await _vpn.startVPN()` попадал в race: `startVPN` приходил в `onStartCommand` guard `if (status != Stopped) return START_NOT_STICKY` — silent-return без `setStatus(Starting)` broadcast'а.

Fix: `BoxVpnService.stopAwait()` возвращает `Deferred<Unit>`, который completes в `setStatus(Stopped)` (после полного cleanup'а). Plugin handler — `withTimeout(5s).await` перед `result.success(...)`. Caller получает control только после реального завершения. Finality-signal через direct Completer — не зависит от доставки broadcast'а (который мы же признали unreliable).

**3. Intent-based sticky reset.** `configStaleSinceStart` сбрасывался только в `_handleStatusEvent` на transition events. Если event потерялся по любой причине (Doze, system kill, platform race) — флаг застревал, плашка висела. Теперь `_stopInternal`/`_startInternal` сбрасывают его **по факту applied intent'а**: stop = «running теперь nothing, saved не stale vs running»; start = «running = saved». Reset в `_handleStatusEvent` остаётся как defense-in-depth для external transitions (revoke, kill).

**4. Reconnect = композиция `_stopInternal` + `_startInternal`.** Убраны `firstWhere`, `timeout(10s, onTimeout: empty map)`, координация wait на Dart стороне. Blocking stopVPN гарантирует `status=Stopped` до `startVPN`, race исключён на native.

Отчёты: `docs/spec/tasks/001-reconnect-sink-leak.md`, `002-blocking-stopvpn-intent-reset.md`.

## 🛡 Revoke UX — friendly handling «VPN taken by another app»

Когда другое VPN-приложение захватывает туннель (Hiddify, WireGuard, FortiClient, …), Android вызывает `VpnService.onRevoke()` на нашем сервисе. Раньше это отражалось красной пилюлей «Revoked by another VPN» в status chip — пугающе и непонятно что делать.

Теперь:

- **SnackBar** «VPN taken by another app» с быстрой action **«Start»** на 5 секунд. Тап по Start → наш existing `VpnPlugin.startVpn` путь: `VpnService.prepare` (занятый слот → non-null intent) → system dialog «VPN connection request» → approve → конкурент получает свой onRevoke → мы занимаем слот.
- **Status chip** показывает обычный Disconnected — видна нейтральная Start кнопка без алармирующего красного. Внутреннее значение `state.tunnel == revoked` сохраняется для side-effect detection в listener'е.
- **Clash endpoint** прошлой сессии обнуляется в `_handleStatusEvent` revoked-ветке — secret был у убитого sing-box, port мог быть переиспользован. Пересоздаётся на следующем `connected` через `_refreshClashAfterTunnel`.

Отчёт: `docs/spec/tasks/003-revoke-ux.md`.

## 🔁 Lifecycle resume re-sync

Event-driven страховка от platform-level потерь broadcast'ов: при возврате в app (`AppLifecycleState.resumed`) — один `getVpnStatus` pull, сравнение с Dart state, если divergent — прогон raw status через тот же `_handleStatusEvent`. Покрывает случаи когда процесс был в background долго, Doze/OOM killer убил service силой без уведомления Flutter и т.п. Если native вернулся с `Revoked` — сработает тот же snackbar-flow что при live-revoke (P2 выше).

В steady-state ничего не крутится — **никакого polling'а/таймеров**. Только одноразовый pull при resume.

Отчёт: `docs/spec/tasks/004-lifecycle-resume-resync.md`.

## 🔄 Reload button (справа от status chip)

Короткий тап — умный default по состоянию (`Connect` / `Reconnect` / `Rebuild config + reconnect`). Long-press — меню из трёх явных действий: `Reconnect`, `Rebuild config only`, `Rebuild config + reconnect`. Dirty-подсветка сохраняется (primary-container фон когда конфиг изменялся при активном VPN).
