...
### ⏰ WARP nodes recover right after the screen turns on
...
but ping measurements taken 5–35 seconds after waking still showed errors.

The screen turning on now serves as a "the device woke up" signal (the screen,
not the unlock — apps hit the network immediately, and the signal has to land
before the first traffic demand): the core walks the nodes and re-establishes
only the provably dead sessions, leaving live ones alone — the error window
should shrink to a single handshake. If WARP still errors out right after
waking, a report is worth sending: this one is hard to reproduce on demand,
and field measurements help.
...
### ⏰ Узлы WARP оживают сразу после включения экрана
...
пинга в первые 5–35 секунд после пробуждения всё равно показывали ошибку.

Теперь включение экрана служит сигналом «устройство проснулось» (именно
включение, а не разблокировка — приложения лезут в сеть сразу, и сигнал должен
успеть до первого спроса трафика): ядро проходит по узлам и переустанавливает
только доказуемо мёртвые сессии, живые не трогает — окно ошибок должно сжаться
до одного рукопожатия. Если WARP всё ещё отдаёт ошибку сразу после пробуждения,
сообщение будет кстати: воспроизвести это по заказу тяжело, полевые замеры
помогают.
