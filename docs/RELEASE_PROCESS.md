...
Поэтому §390: приложение знает свой канал (`--dart-define=LXBOX_DISTRIBUTION`,
проставляется в CI) и уведомление об апдейте ведёт в тот же магазин, откуда
приложение пришло. При правке шагов сборки в `ci.yml` флаг не терять: APK-шаги
несут `=github`, AAB-шаг — `=play`.
