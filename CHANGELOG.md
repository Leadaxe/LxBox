...
Одно исправление: ANR («Приложение не отвечает») при перезагрузке конфига на
работающем туннеле (§373). Пришло из жалобы пользователя с диагностическим
дампом — в трейсе главный поток стоял прямо на `startOrReloadService`.
Проявлялось на больших подписках: дефект безусловный, но пороговый, и на
типовом конфиге перезагрузка не дотягивала до пяти секунд.

Ядро не менялось — пин остаётся `v1.14.0-lx.20-rc.3`. Причина была на стороне
приложения: блокирующий вызов ядра шёл с потока интерфейса.
