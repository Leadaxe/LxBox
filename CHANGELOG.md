...
- **Импорт LX Backup формата 1.0 ([§438](docs/spec/tasks/438-lx-backup-1-0-read.md)).**
  Было: файл лаунчера формата 1.0 (`lx_backup: 2`) отвергался как «новее
  поддерживаемого». Стало: импорт читает оба формата — 0.12 и 1.0 — и сливает
  их одним кодом. Из файла 1.0 приезжают подписки, корневые узлы, папки со
  своим составом и настройками, цепочки, правила маршрута в форме sing-box
  (включая `reject` с `method: drop` и самостоятельные `action` — видом json),
  DNS-серверы и правила вместе с `default_domain_resolver`, секции узлов
  (правила и DNS, которые узел носит с собой). Папка сопоставляется сперва по
  `id`, затем по имени — только среди папок, которые уже были на устройстве, так
  что две папки с одинаковым именем больше не сливаются в одну. Preset-серверы
  DNS сопоставляются по ссылке, а не по тегу. Правила узлов встают на общую ось
  порядка вместе с корневыми. Цепочка и провайдерская группа внутри папки на
  телефоне не хранятся — такая запись называется предупреждением. Экспорт
  по-прежнему пишет формат 0.12.
...
- **Выпуск в Google Play уезжает из CI сам ([§436](docs/spec/tasks/436-google-play-upload-ci.md)).**
  ...
  Релиз-кандидаты `vX.Y.Z-rc.N` в Play не уходят, hotfix-теги уходят.
