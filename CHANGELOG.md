...
- **§331 — плашка «Настройки изменены» больше не появляется от неудачного
  обновления подписки.** Ревью нашло обходной путь той же жалобы: неудачный фетч
  (провайдер недоступен, авиарежим) записывал служебный статус попытки и этим
  поднимал плашку — раз в час, на конфиге, который никто не менял. Служебные
  записи больше флаг не трогают; заодно закрыта гонка, при которой обновление
  подписки могло погасить отметку о настоящей несохранённой правке, сделанной
  в момент обновления.
- **§331 — плашка «Настройки изменены» больше не появляется после обновления
  подписки, которая не изменилась.** Флаг «есть несохранённые изменения»
  поднимался на любую запись настроек, а обновление подписки пишет их всегда —
  хотя бы отметку времени последней попытки. Подписка, вернувшая тот же список
  узлов, предлагала пересобрать конфиг, которому нечего менять. Теперь сравнивается
  состав: узлы (с учётом их порядка) и отметки отключённых. Если ранее были
  настоящие несохранённые изменения — обновление подписки их плашку не гасит.
...
- **§324 — снимок работающего конфига (§311) не доходил до приложения вообще.**
  Обвязка отдавала во Flutter объект вместо строки — с обновления ядра до
  `lx.17-rc.1` метод возвращает `RunningConfig`, а не текст, и вызов `.content()`
  забыли. Канал молча возвращал пустоту, поэтому «View details» для узла, resolve
  тегов и всё, что должно показывать состояние *запущенного* ядра, тихо
  откатывалось на сохранённый файл. Заметно это было только в момент, когда
  конфиг на диске уже разошёлся с работающим.
...
Ядро: **sing-box-lx `v1.14.0-lx.17-rc.3`** (без изменений).
