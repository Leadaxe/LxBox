...
- **§372 — импорт из файла на устройствах без файлового менеджера.** На
  Android TV системного документ-пикера обычно нет: нажатие «импорт из файла»
  заканчивалось техническим сообщением «Can't handle the provided file type»
  или ничем. Теперь приложение распознаёт такую ситуацию и подсказывает
  рабочую альтернативу — вставку из буфера обмена или добавление по ссылке.
