...
- **Узел Tailscale и «секции узла» ([§435](docs/spec/features/435%20node-sections-tailscale/spec.md), контракт ## 13).**
  ...
  секции до контракта 1.0 не едут (экспорт называет потерю).
...
- **Ядро обновлено до `v1.14.0-lx.36`** (пин был `lx.34`; [docs/KERNEL.md](docs/KERNEL.md)).
  База апстрима та же (sing-box 1.14.0 + 16 коммитов), конфиги и провод не
  меняются. `lx.35` — хотфикс issue #14 ядра: XHTTP за CDN, сбрасывающим
