...
- **Лист «N servers disabled» ведёт в детали узла на вкладке Notifications
  ...
  Notifications; удалённый узел в листе неактивен. Заголовок плашки и листа:
...
- **Уведомления узла — отдельная вкладка в деталях
  ...
  только там. Теперь на экране разбора узла подписки и в настройках
  одиночного сервера / члена папки есть вкладка Notifications (перед
  Diagnostics): счётчик и значок старшего уровня на подписи, внутри — тот же
  список карточек, что в шторке; без уведомлений — одна строка «No
  notifications for this server». Тап по значку в списке не изменился.
