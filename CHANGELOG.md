...
- **Ядро `v1.14.1-lx.4` → `v1.14.1-lx.5` ([§461](docs/spec/tasks/461-kernel-lx5-short-id-hotfix.md)).**
  Hotfix SPEC 090: REALITY `short_id` длиннее 16 hex-символов теперь отвергается
  ...
  результат `hex.Decode` в массив `[8]byte`, не проверив длину входа. Провод,
  схема конфига, наборы тегов AAR, Go-тулчейн и сабмодули без изменений;
  Java-поверхность совпадает с lx.4 (javap по всем 253 классам — дифф пуст).
  Гард §343 на стороне приложения остаётся: слишком длинный `short_id`
  по-прежнему отбрасывается при разборе.
