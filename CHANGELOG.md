...
- **Экспорт и импорт правил роутинга файлом (§396).** На вкладке Rules
  появилось меню ⋮: «Export rules...» открывает окно выбора правил (по
  умолчанию отмечены все) и сохраняет их одним JSON-файлом через знакомый по
  бэкапу шит — «Сохранить в файл» / «В Загрузки» / «Поделиться»; «Import
