...
- **Quick Connect: QS tile + home-screen shortcut** ([§032](docs/spec/features/032%20quick%20connect/spec.md), [#1](https://github.com/Leadaxe/LxBox/issues/1)) — две точки toggle VPN без открытия app'а. **Quick Settings tile** в шторке: тап = on/off, плитка живёт `Active/Inactive` синхронно с реальным статусом сервиса (`BoxVpnService.setStatus → TileService.requestListeningState`). **Home-screen shortcut** «Toggle VPN» — long-press на иконку app'а, статический `res/xml/shortcuts.xml` с `extra action=toggle`. Оба пути идут через `MainActivity.handleQuickAction` и шарят VPN-consent flow: первый раз `MainActivity` коротко мелькает ради `VpnService.prepare(...)` (Android API ограничение — consent можно показать только из Activity), показывает toast «Opening L×Box for VPN permission (one-time)», после `RESULT_OK` стартует сервис и `finish()`'ится — юзер возвращается на хоум. App Settings → General → **Quick connect** добавляет блок с кнопкой `Add` (на API 33+ зовёт `StatusBarManager.requestAddTileService`, на старых — текстовая инструкция drag из шторки) и подсказку про long-press на иконку. Edge cases: tap во время `Starting`/`Stopping` молча игнорится; `onDestroy` сбрасывает `currentStatus → Stopped` если сервис умер не через doStop (OOM-kill), чтобы tile не врал «Connected». См. [task 014](docs/spec/tasks/014-quick-connect-tile-shortcut.md).
