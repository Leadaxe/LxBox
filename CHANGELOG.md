...
- **Support-лента: сеть только с согласия на проверку обновлений ([§422](docs/spec/tasks/422-support-feed-gated-by-update-consent.md)).**
  ...
  запроса за `docs/support.json`. Что было → стало:
  ...
  | без сети и без кэша лента молчит | в APK вшита копия `docs/support.json` на момент сборки — очередь и пороги работают с первого запуска; в кэш она не пишется, при появлении согласия придёт свежая |
  | копия могла разъехаться с источником (как `donate.json`) | тест паритета `assets/support.json == docs/support.json`, CI красный при расхождении |
