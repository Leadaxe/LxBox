...
- **§380 — детерминированный build-id для нативных `.so`.** Линкер штампует
  в каждую библиотеку секцию `.note.gnu.build-id`, по умолчанию уникальную для
  каждого прогона: два прогона одного исходника расходятся, и побайтовая
  верификация F-Droid проваливается, не начавшись. Хук на подпроекты в
  `app/android/build.gradle.kts` задаёт `--build-id=sha1` всем, кто собирает
  нативный код через CMake (`flutter_zxing`, `jni`). Идентификатор считается
  от содержимого файла, поэтому одинаковый вход даёт одинаковый результат.

  Выбран `sha1`, а не `none`: секция системная — по ней символизируются
  тумбстоуны, работают crash-репортеры и `debuginfod`. Выбрасывать её ради
  верификации не потребовалось.

  Замер до правки (arm64, F-Droid против GitHub): расходились 3 файла из 379.
  `libapp.so` — из-за абсолютного пути `dart_plugin_registrant.dart` внутри
  Dart-снапшота (лечится на стороне рецепта, сборка из того же каталога, что
  и GitHub Actions — приём 78 приложений каталога); `libdartjni.so` и
  `libflutter_zxing.so` — как раз из-за build-id. Поведение приложения не
  менялось.
