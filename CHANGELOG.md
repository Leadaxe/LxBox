...
- **Узлы trojan из ссылок разбираются общим конвейером, и предупреждений у
  них стало больше — с адресом поля
  ([§472](docs/spec/features/472%20unified-parse-pipeline/spec.md), шаг 2).**
  ...
  судит импорт JSON. Раньше правила жили в самом парсере trojan, своей копией.
  ...
  пришедший телом. Тела узлов, их имена и идентичность не изменились —
  выбор узла, отключения и цепочки на месте. Остальные протоколы разбираются
