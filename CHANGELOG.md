...
- **LX Backup формата 1.0: запись и чтение ([§438](docs/spec/tasks/438-lx-backup-1-0-read-write.md)).**
  ...
  В файл теперь едут то, что 0.12 терял: папки со своими настройками и
  составом (включая нечитаемых членов), секции узлов (правила и DNS, которые
  узел носит с собой), личный detour узла, правила вида json (как правила с
  телом sing-box), DNS `default_domain_resolver`. Позиции цепочек и detour на
  члена папки или узел подписки едут ссылкой на папку/подписку, а не тегом с
  префиксом. Не выражаются в 1.0 и называются при экспорте: import-rules и
  реакция на обновление подписки, политика detour подписки и папки, ping-опции
  папки, имя цепочки, srs-правила DNS.

  Из файла 1.0 приезжают подписки, узлы, папки, цепочки, правила маршрута
  (включая `reject` с `method: drop` и самостоятельные `action`), DNS и секции
  узлов. Папка сопоставляется сперва по `id`, затем по имени — только среди уже
  существовавших, поэтому две папки с одинаковым именем больше не сливаются, а
  повторный импорт собственного экспорта ничего не задваивает. Preset-серверы
  DNS сопоставляются по ссылке `<preset>:<tag>`. Номера правил из файла
  сохраняются: правило, добавленное после импорта, встаёт перед широкими
  пресетами, как и раньше. Цепочка и провайдерская группа внутри папки на
  телефоне не хранятся — такая запись называется предупреждением. Префикс тегов
  переводится между сторонами: у лаунчера разделитель входит в префикс.
