...
VLESS с шифрованием больше не снимается, узлы sing-box JSON и проверка
задержки проходят через реестр протоколов. Контракт 1.1.55.
...
- **Эмиттеры sing-box больше не держат копий правил реестра ([§546](docs/spec/tasks/546-emitters-drop-registry-rule-copies.md)).**
  ...
  правка реестра теперь доходит до результата без правки кода. Остались
  связь `uplink_data_placement`↔`mode` у XHTTP и `plugin_opts`↔`plugin` у
  Shadowsocks — в реестре для них правила пока нет.
