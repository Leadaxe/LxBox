...
- **Четыре предупреждения узла читают текст из реестра контракта, а не из
  кода ([§482](docs/spec/tasks/482-warnings-text-from-registry.md)).**
  `ech_ignored`, `ws_early_data_converted`, `naive_padding_ignored` и
  `naive_extra_headers_invalid` держали свой текст в приложении отдельно от
  нормативного — вторая таблица рядом с общей расходилась бы с ней молча.
  Теперь у них тот же источник, что у остальных кодов: заголовок, развёрнутое
  объяснение, причина и совет приходят из `warnings.json`, поэтому правка у
  лаунчера доезжает до обеих сторон сразу. Уровень (`info`) и адрес поля не
  изменились. Два кода AmneziaWG (`awg_header_invalid`, `awg3_field_invalid`)
  пока остаются со своим текстом: реестровый не объясняет последствия для
  рукопожатия.
...
- **Предупреждения узла стали «Уведомлениями»: уровни, счётчики и
  ...
  Errors / Warnings / Info, запись раскрывается в путь поля, «Что произошло»,
  «Почему так», «Что можно сделать» и ссылку «Подробнее». Тот же список — на
  экране узла разделом Notifications внизу вкладки Settings, вместо прежней
...
- **Сервер, который не принимает ядро, выключается сам, и VPN поднимается на
  ...
  виноват и что с ним делать. Стало: по отказу приложение разбирает строку,
  ...
  появляется плашка «Отключено серверов: N» с кнопкой **Show** — списком с
...
- **Тап по ⚠ под узлом открывает карточку: почему так вышло и что сделать
  ...
  «Подробнее» открывает в браузере страницу про этот код целиком.
  Тексты лежат в реестре и показываются офлайн: наружу ведёт только
  «Подробнее». Ссылка идёт на нашу копию документации контракта
...
- **Копия документации контракта в репозитории
  ...
  байт в байт в `docs/contract/` — туда ведёт «Подробнее» с карточки. Своего
