...
- **§090 G2a — Detour-фильтр по факту ссылок** ([task spec](docs/spec/tasks/093-detour-by-isdetour.md)). Тогл «Show detour» на главном теперь прячет/показывает ноды по тому, **используются ли они реально как хоп** (на них ссылаются через `detour` — `ConfigNode.isDetour`), а не по ручной ⚙-метке. «Detour-сервер» = релей, через который ходят другие ноды.
