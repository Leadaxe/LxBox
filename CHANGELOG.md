...
- **§207 — захват pprof-профиля на устройстве (goroutine + CPU)** ([task spec](docs/spec/tasks/207-goroutine-cpu-dump.md), [PProfClient.kt](app/android/app/src/main/kotlin/com/leadaxe/lxbox/vpn/PProfClient.kt) + [box_vpn_client.dart](app/lib/vpn/box_vpn_client.dart) + [profile_dump_writer.dart](app/lib/services/profile_dump_writer.dart) + [diagnostics_tab.dart](app/lib/screens/app_settings_screen/widgets/diagnostics_tab.dart) + [debug_screen.dart](app/lib/screens/debug_screen.dart) + [diag.dart](app/lib/services/debug/handlers/diag.dart)). Кнопки **«Capture goroutine dump»** / **«Capture CPU profile (10s)»** снимают pprof-слепки с живого ядра sing-box и открывают системный Share (`.txt` / `.pb`, анализ — `go tool pprof`). Цель — диагностика нагрева/100 % CPU: CPU-профиль ловит busy-spin в tight-loop, goroutine-дамп показывает стеки всех горутин (раньше был только **счётчик** `goroutines`, без трасс). Реализовано целиком на стороне оболочки через встроенный в libbox `PProfServer` (Go `net/http/pprof`) — **ядро не правилось**: сервер поднимается по тапу на loopback-порту (первый свободный из 6060..6065), отдаёт один GET, гасится в `finally` (в проде http-listener не висит). Один сервер бесплатно отдаёт весь набор `/debug/pprof/*` → обобщённый Debug API `GET /diag/pprof?profile=goroutine|profile|heap|allocs|block|mutex|threadcreate&seconds=N`; goroutine-стек вкладывается в `/diag/dump` при активном туннеле. Обе кнопки гейтятся на активный VPN + re-entrancy guard (двойной тап не поднимет два сервера на одном порту). Грабля: Android API 28+ режет cleartext HTTP даже на 127.0.0.1 → `network_security_config.xml` разрешает cleartext **только для loopback** (внешний трафик остаётся под дефолтом). Device-verified (CPH2411, rc.14): `/diag/pprof?profile=goroutine` → 130 горутин. +6 тестов.
