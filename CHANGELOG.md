...
- **Узлы trojan и vless из ссылок разбираются общим конвейером, и
  предупреждений у них стало больше — с адресом поля
  ([§472](docs/spec/features/472%20unified-parse-pipeline/spec.md), шаги 2
  и 3).** Внутренняя перестройка: ссылка теперь переводится в карту sing-box
  ...
  набора XHTTP. Тела узлов, их имена и идентичность не изменились — выбор
  узла, отключения и цепочки на месте. Остальные протоколы разбираются
  по-прежнему; они переезжают следующими шагами.
...
- **Настройки TCP keep-alive узла (§453) не пропадают при разборе ссылки
  trojan.** Шаг 2 отдавал их санитайзеру контракта, который таких полей не
  знает (лаунчер их не пишет) и снимал их вместе с предупреждением
  «неизвестный ключ» о настройке самого приложения. Теперь они идут мимо
  санитайзера — у trojan и у vless одинаково. Проявлялось только в приложении:
  в тестах реестр не был загружен, и санитайзер там не работал.
