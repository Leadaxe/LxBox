...
- **Каналы переименованы в Направления (SPEC 393)**: сквозно — модель, UI
  (экраны, меню, тексты), внутренний и LX-бэкап. Ключ хранилища
  `channels`→`directions` с одноразовой миграцией (данные переезжают сами).
  Debug API: `/channels` → `/directions` **без алиасов** — автоматизацию
  надо поправить. Теги `vpn-N` остаются валидными, потолок в 10 снят.
- **Цепочки хопов (SPEC 110)**: третий вид источника — маршрут через 2+
  серверов; редактор с d&d и валидацией инвариантов старта ядра; послойная
  диагностика с ценой каждого хопа; heal позиций при удалении источника.
- **Направления**: произвольные теги (потолок 10 снят), include[] других
  Направлений, перегруппированная форма, каскад смены префикса подписки.
- **Живое применение**: правки источников при активном туннеле применяются
  in-place reload'ом без баннера «перезапустите VPN».
- **LX Backup (контракт 0.7.1)**: корневые секции directions[] и chains[]
  (правила приезжают рабочими), warp[] и dns в обе стороны, применение
  vars/route.final/подписок на импорте, чужие extensions переживают круг.
- **Санитайзер графа**: битый элемент деградирует с предупреждением вместо
  отказа всего конфига; пустое Направление блокирует, а не течёт мимо VPN.
...
- **Перенос настроек между телефоном и десктопом (LX Backup v1).**
  *Резервные копии → Перенос на десктоп.* Общий формат обмена с десктопным
  лаунчером: подписки, серверы, правила, DNS и
  переносимые переменные едут одним файлом. То, чего на десктопе нет —
  матчеры по приложениям и Wi-Fi, папки — уезжает в раздел расширений и
  возвращается обратно нетронутым: настройка, побывавшая на десктопе, не
  должна вернуться обеднённой.
  ...
  правило с мёртвой целью роняет конфиг ядра целиком. Настройка, которая на
  другой машине означает другое (пути, интерфейсы), не применяется, и об
...
- **Ядро обновлено до `v1.14.0-lx.30`** (было `v1.14.0-lx.28-rc.1`). Починен
  ...
  утечка пула в `stream-up`. Из lx.28: у MASQUE `vhttp: auto` стал дефолтом —
  за TCP-only-хопом туннель спасается в h2 вместо зависания до дедлайна.
  Java-поверхность ядра изменилась только дополнениями (RPC цепочек, клиент
  их не биндит) — правок обвязки не потребовалось.
...
- **Теги внутри пресетов получили неймспейс `<пресет>:<тег>`.** Раньше два
  ...
  шаблона, а не пользователь.

  Ссылки в уже сохранённых настройках переезжают на новые теги
  автоматически при первой же сборке конфига. Неоднозначные (когда два
  пресета объявили одинаковое имя) не угадываются — такая ссылка снимается
  штатной деградацией, как и любая другая битая.
...
- **Ядро обновлено до `v1.14.0-lx.27-rc.4`** (было `rc.2`). Добит фриз detour
  (SPEC 072): провал поднятия XHTTP-стрима рвёт upload-пайп с читающей половины,
  а запросы всех трёх режимов ездят на conn-scoped контексте — 15-секундный
  дедлайн дайла больше не рвёт живой стрим. Полевой симптом, который это чинит:
  весь трафик мёртв (включая direct), пинг-тест при этом проходит, лечит только
  force-stop. Java API ядра не изменился.

### Fixed

- **XHTTP-узлы из JSON-подписок собирались с урезанным транспортом (§399).**
