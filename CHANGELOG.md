...
- **Узлы trojan, vless, vmess, shadowsocks, hysteria2 и tuic из ссылок
  разбираются общим конвейером, и предупреждений у них стало больше — с
  адресом поля
  ([§472](docs/spec/features/472%20unified-parse-pipeline/spec.md), шаги 2–5).**
  ...
  перегрузки и режиме передачи UDP. Тела узлов, их имена и идентичность не
  изменились — выбор узла, отключения и цепочки на месте. Остальные протоколы
  разбираются по-прежнему; они переезжают следующими шагами.
